...
<div class="flex-1 min-h-screen p-4">  
  <!-- Content -->
  <div class="container mx-auto mb-4  z-0">
    <!-- IRS Content -->
    <div class=" overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400  z-0">
                    <tr>
                ...
                <tbody action="">
                    @foreach ($verifIRS as $key => $verifirs)
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifirs->mahasiswa->nama }}</td> 
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifirs->NIM }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifirs->mahasiswa->angkatan }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifirs->smst_aktif }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifirs->jumlah_sks }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">
                                <a href="#" type="button" class="text-black-600 hover:underline dark:text-black-500 btn btn-link view-irs" data-scan-url="{{ asset($verifirs->berkas_irs) }}">View IRS</a>
                            </td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">
                                <button onclick="window.location.href='{{ route('tolak_verifikasi_irs', ['idIrs' => $verifirs->id_irs]) }}'" type="button" class="text-white bg-gradient-to-r hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                                ...
                                <button onclick="window.location.href='{{ route('editIrs_dosenwali', ['idIrs' => $verifirs->id_irs]) }}'" type="button" class="text-white bg-gradient-to-r hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                                    <svg xmlns="http://www.wwali'3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0,0,256,256">
                                ...
                                <button onclick="window.location.href='{{ route('setujui_verifikasi_irs', ['idIrs' => $verifirs->id_irs]) }}'" type="button" class="text-white bg-gradient-to-r hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0,0,256,256">
        ...
        <!-- Modal structure -->
        <div class="modal" id="irsModal">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">View IRS</h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <iframe id="pdfViewer" src="{{ asset('storage/irs/'.$verifirs->berkas_irs)}}" width="100%" height="500px"></iframe>
                    </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                var irsButtons = document.querySelectorAll('.view-irs');
                var pdfViewer = document.getElementById('pdfViewer');

                irsButtons.forEach(function (button) {
