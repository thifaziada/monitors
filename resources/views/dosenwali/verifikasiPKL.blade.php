  ...
  <div class="container mx-auto mb-4">
    ...
    <div class=" overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody action="">
                    @foreach ($verifPKL as $key => $verifpkl)
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifpkl->mahasiswa->nama }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifpkl->NIM }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifpkl->mahasiswa->angkatan }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifpkl->status_pkl }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">{{ $verifpkl->nilai_pkl }}</td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">
                                <a href="#" type="button" class="text-black-600 hover:underline dark:text-black-500 btn btn-link view-pkl" data-scan-url="{{ asset($verifpkl->berkas_PKL) }}">View PKL</a>
                            </td>
                            <td class="px-6 py-4 text-center whitespace-no-wrap">
                                <button onclick="window.location.href='{{ route('tolak_verifikasi_pkl', ['idPkl' => $verifpkl->id_pkl]) }}'" type="button" class="text-white bg-gradient-to-r hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                                ...
                                <button onclick="window.location.href='{{ route('editPkl_dosenwali', ['idPkl' => $verifpkl->id_pkl]) }}'" type="button" class="text-white bg-gradient-to-r hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0,0,256,256">
                                ...
                                <button onclick="window.location.href='{{ route('setujui_verifikasi_pkl', ['idPkl' => $verifpkl->id_pkl]) }}'" method="POST" type="button" class="text-white bg-gradient-to-r hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0,0,256,256">
        ...
        <!-- Modal structure -->
        <div class="modal" id="pklModal">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">View PKL</h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <iframe id="pdfViewer" src="{{ asset('storage/PKL/'.$verifpkl->berkas_pkl)}}" width="100%" height="500px"></iframe>
                    </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                var pklButtons = document.querySelectorAll('.view-pkl');
                var pdfViewer = document.getElementById('pdfViewer');

                pklButtons.forEach(function (button) {
